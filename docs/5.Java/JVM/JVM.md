...
能观察到状态为terminated的线程吗？

线程池里的线程，执行完任务后，是什么状态？
...
JVM是一个虚拟机软件，这个虚拟机软件可以执行字节码指令。

字节码，指的是一串具有特定格式的二进制流，它可以是硬盘上的.class文件，也可以是内存中的一个byte[]数组。

字节码是一种平台无关的格式，与操作系统和CPU指令集都没有关系；
